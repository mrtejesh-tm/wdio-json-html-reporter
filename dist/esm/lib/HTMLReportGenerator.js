...
<body>
  ...
  <script>
    ...
    function generateSuiteTestsTable(tests) {
      ...
      // Use the unified table building function without suite column
      return buildTableHTML(tests, { showSuiteColumn: false, includePagination: false });
    }    
    ...
    function renderTable(page) {
      ...
      // Use the unified table building function
      const tableHTML = buildTableHTML(currentData, { showSuiteColumn: true });
    ...
    function buildTableHTML(tests, options = {}) {
      const { showSuiteColumn = true, includePagination = true } = options;
      ...
      if (showSuiteColumn) {
        html += '<th>Suite Name <span class="filter-icon" onclick="openFilterMenu(event, 1)">&#x1F50D;</span></th>';
      }
  
      html += '<th>Test Name <span class="filter-icon" onclick="openFilterMenu(event, 2)">&#x1F50D;</span></th>' +
        '<th>Status <span class="filter-icon" onclick="openFilterMenu(event, 3)">&#x1F50D;</span></th>' +
        '<th>Error</th>' +
        '<th>Screenshot</th>' +
        '<th>Expected Result</th>' +
    ...
    let rowAttributes = '';
    if (showSuiteColumn) {
      rowAttributes = 'data-suite="' + escapeHtml(test.suiteName || '') + '" data-test-name="' + escapeHtml(test.testName || '') + '" data-status="' + (test.status || '') + '" data-error="' + escapeHtml(test.errors ? test.errors.map(e => e.message).join(' | ') : '') + '"';
    }
    ...
    if (showSuiteColumn) {
      ...
      html += '<td class="suite-cell">' + suiteStatusEmoji + ' ' + escapeHtml(test.suiteName || '') + '</td>';
    }
    
    html += '<td>' + escapeHtml(test.testName || '') + '</td>' +
