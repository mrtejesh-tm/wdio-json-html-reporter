...
export default class JSONReporter extends WDIOReporter {
  ...
  async addTestResult(test, status) {
    ...
    const timestamp = date.toUTCString(); // human-readable UTC timestamp
    ...
    // Remove any occurrence of "suite" followed by numbers (e.g., suite1, suite2)
    ...
    if (
      ...
      try {
        ...
        // Generate a short unique identifier using a simple hash function.
  ...
  writeJSONReport() {
    ...
    // Create a safe timestamp for the file name using UTC time.
  ...
  /**
   * Helper method to create a file name safe timestamp string from a Date object.
   * It removes commas, replaces colons and spaces with safe characters.
   */
   ...
   * This is used to keep screenshot filenames short and unique.
   */
  generateShortHash(input) {
    ...
    // Convert the absolute value to hex and ensure it's at least 8 characters.
