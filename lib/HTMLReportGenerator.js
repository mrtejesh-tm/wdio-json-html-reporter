...
<body>
  <div class="header">
    ...
    <div class="execution-subheader">
      <p>Browser Name: ${metadata.browserName} | Start: ${metadata.executionStartTime} | End: ${metadata.executionEndTime} | Duration (mins): ${metadata.totalTimeInMinutes}</p>
    </div>
  </div>
  <div class="nav">
  ...
  <script>
    ...
    function exportTableToExcel(tableID, filename = '') {
      ...
      var style = \`
      <style>
        table { border-collapse: collapse; width: 100%; font-family: var(--primary-font); }
        table, th, td { border: 1px solid #e0e0e0; }
        th, td { padding: 12px; text-align: center; word-wrap: break-word; white-space: normal; }
        th { background-color: var(--table-header-bg); color: #fff; }
...
\${htmlTable}
  ...
  statusChartInstance = new Chart(ctx, {
    ...
    plugins: [ChartDataLabels] // Moved outside options
  });
}
