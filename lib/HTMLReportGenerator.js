  ...
  generateHTMLReport(historyData) {
    ...
    if (Object.keys(historicalBySuite).length > 0) {
      ...
      Object.keys(historicalBySuite).forEach((suite) => {
        ...
        let redDot = latestRecord.newIssues && latestRecord.newIssues.length > 0 ? '<span class="dot red-dot"></span>' : '';
        let greenDot = latestRecord.resolvedIssues && latestRecord.resolvedIssues.length > 0 ? '<span class="dot green-dot"></span>' : '';
    ...
    // Create a container to hold the Unique Errors table and the historical accordion side-by-side.
    const tablesContainerHTML = `
    ...
    // Include a chart container for history trends.
...
<head>
  ...
  <style>
    ...
    /* Scrollable Table for Historical Details */
    .scrollable-table {
      max-height: 300px;
      overflow-y: auto;
      overflow-x: auto;
  ...
  <script>
    ...
    function exportTableToExcel(tableID, filename = '') {
      ...
      <style>
        ...
        th { background-color: rgb(124, 182, 249); color: #2c3e50; }
        td.passed { background-color: rgb(240, 250, 238); color: rgb(4, 60, 6); }
        td.failed { background-color: rgb(232, 203, 195); color: rgb(155, 0, 15); }
      </style>\`;
