...
export default class HTMLReportGenerator {
  ...
  generateHTMLReport() {
    ...
    // Build Unique Errors table rows with SI NO and a data attribute holding the raw error message.
    ...
    const uniqueErrorsRows = uniqueErrors.map((errMsg, index) => {
      ...
      // Use encodeURIComponent to safely store the raw error message in a data attribute.
...
<body>
  ...
  <script>
    ...
    // Expose suiteStats to client-side for filtering logic
    window.suiteStats = ${JSON.stringify(suiteStats)};
    
    // Function to navigate to tests with the selected error.
    ...
    // After DOM content is loaded, attach click listeners to unique error rows.
    document.addEventListener("DOMContentLoaded", function() {
      // Attach click listener for error rows in Unique Errors table.
    ...
    // When opening a filter menu for a column, compute available values from the dataset filtered by all active filters except the current column.
    function openFilterMenu(event, colIndex) {
      ...
      // Compute unique values from the dataset for the current column
...
\${htmlTable}
    ...
    // New function to toggle the display of the stack trace when an error message is clicked.
    ...
    // New function: filter by Passed Suites (suites with zero failures)
