...
<body>
  ...
  <script>
    ...
    // Combined Filtering: single text input (searching Suite Name) + column dropdown filters.
    function applyFilters() {
      ...
      for (let row of rows) {
        ...
        let textMatch = (suiteText.indexOf(suiteFilterText) > -1);
    ...
    // Updated Export table to Excel functionality without screenshots and filter icons, with enhanced styling.
    function exportTableToExcel(tableID, filename = '') {
      var dataType = 'application/vnd.ms-excel';
      var tableSelect = document.getElementById(tableID);
      var clonedTable = tableSelect.cloneNode(true);
      
      // Remove screenshot column (last column) from header and rows
      ...
      // Remove filter icons from header cells
      clonedTable.querySelectorAll("th .filter-icon").forEach(el => {
        el.remove();
      });
      
      // Remove onclick attributes from header cells for a cleaner export
      clonedTable.querySelectorAll("th").forEach(th => {
        th.removeAttribute("onclick");
      });
      
      // Add inline styling for a professional look in Excel including the status colors.
      var style = \`
      <style>
